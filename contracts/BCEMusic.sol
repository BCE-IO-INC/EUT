...
contract BCEMusic is ERC1155, Ownable, ReentrancyGuard, IBCEMusic {
    ...
    function startAuction(uint256 tokenId, uint16 amount, uint16 minimumBidAmount, uint16 bidUnit, uint256 reservePricePerUnit, uint256 biddingPeriodSeconds, uint256 revealingPeriodSeconds, AuctionPricingPolicy pricingPolicy) external override returns (uint64) {
        uint balance = balanceOf(msg.sender, tokenId);
        ...
        uint64 auctionId = BCEMusicAuction.startAuction(msg.sender, auctions, amount, minimumBidAmount, bidUnit, reservePricePerUnit, biddingPeriodSeconds, revealingPeriodSeconds, uint8(pricingPolicy));
